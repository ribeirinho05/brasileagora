"""
BRASIL AGORA — Orquestrador Principal
...
logger = logging.getLogger('brasileagora')
...
def _git_push_automatico():
    try:
        ...
        subprocess.run(['git', 'commit', '-m', f'Atualização automática BRASIL AGORA - {agora}'], cwd=repo_dir, check=True, capture_output=True)
...
def configurar_logging():
    ...
    log_file = os.path.join(CAMINHO_LOG, f'brasileagora_{datetime.now().strftime("%Y%m%d")}.log')
    logging.basicConfig(
        level=logging.INFO,
...
def executar_atualizacao():
    inicio = datetime.now()
    logger.info("=" * 60)
    logger.info("BRASIL AGORA — Iniciando atualização")
    logger.info(f"Data/hora: {inicio.strftime('%d/%m/%Y %H:%M:%S')}")
    logger.info("=" * 60)

    try:
        logger.info("\n ETAPA 1: Coletando notícias...")
        from coletor_brasilia import coletar_todas_brasilia
        noticias_brasilia = coletar_todas_brasilia()
        logger.info(f"  Brasília/Governo: {len(noticias_brasilia)} notícias")
        ...
        todas = noticias_brasilia + noticias_externas
        ...
        if GEMINI_API_KEY:
            ...
            trending = ['#BrasilAgora', '#Brasília', '#Brasil', '#Economia', '#Política', '#Tecnologia']
