"""
BRASIL AGORA — Coletor de Notícias de Brasília e Governo Federal
Scraping do Planalto, Câmara dos Deputados, Senado e Agência Brasil
"""
...
logger = logging.getLogger('brasileagora.coletor_brasilia')
...
def coletar_todas_brasilia():
    from config import FONTES_GOV
    todas = []

    for fonte_id, fonte in FONTES_GOV.items():
        try:
    ...
    logger.info(f"Total de notícias coletadas de Brasília: {len(todas)}")
    return todas
...
def _coletar_fonte(fonte):
    ...
    links = _extrair_links_noticias(soup, url_base, fonte)
...
def _extrair_links_noticias(soup, url_base, fonte):
    links = set()
    excluir = ['#', 'javascript:', 'mailto:', '/tag/', '/categoria/', '/page/', 'login', 'cadastro', '.pdf', '.jpg', '.png']

    is_camara = 'camara.leg.br' in url_base
    is_senado = 'senado.leg.br' in url_base
    is_govbr = 'gov.br' in url_base
    ...
    for a in soup.find_all('a', href=True):
        ...
        if is_govbr and 'gov.br' in href and '/noticias/' in href:
            links.add(href)
            continue
...
def _extrair_noticia_generica(url, fonte):
    ...
    texto = ''
    for seletor in ['.noticia-texto', '.entry-content', '.post-content', '.news-content', '.conteudo', 'article .text', 'article p', '#content-core']:
