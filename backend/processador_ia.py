"""
BRASIL AGORA — Processador IA (Google Gemini 2.5 Flash via HTTP direto)
Reescreve notícias com linguagem jornalística, gera breaking news e trending topics
"""
...
logger = logging.getLogger('brasileagora.processador_ia')
...
def reescrever_noticia(noticia, model=None):
    ...
    try:
        ...
        if resultado and resultado.get('rejeitar'):
            logger.info(f"  IA rejeitou (não é notícia): {titulo[:60]}...")
        ...
        if resultado and resultado.get('titulo') and resultado.get('resumo'):
            ...
            logger.info(f"  IA reescreveu: {noticia['titulo'][:60]}...")
        else:
            noticia['resumo'] = texto[:300] if texto else titulo
            noticia['ia_processada'] = False
            logger.warning(f"  IA não retornou JSON válido, usando original")
...
def processar_lote(noticias, model=None):
    ...
    for i, noticia in enumerate(noticias):
        ...
        try:
            ...
            quota_esgotada = True
            logger.warning(f"  Quota diária atingida na notícia {i+1}/{total}.")
            noticia['resumo'] = noticia.get('texto', noticia.get('titulo', ''))[:300]
            noticia['ia_processada'] = False
            continue
        time.sleep(1)
    ...
    noticias = [n for n in noticias if not n.get('rejeitada_ia')]
...
def gerar_breaking_news(noticias, model=None):
    ...
    try:
        ...
        if resultado and resultado.get('breaking'):
            breaking = resultado['breaking'][:3]
            logger.info(f"IA gerou {len(breaking)} breaking news")
            return breaking
    ...
    for i, n in enumerate(top_noticias[:3]):
        breaking.append(f"{prefixos[i]} {n.get('titulo', '')[:100]}")
...
def gerar_trending_topics(noticias, model=None):
    ...
    resumo = '\n'.join(
        f"- {n.get('titulo', '')} [{n.get('categoria', '')}]"
    ...
    try:
        ...
        if resultado and resultado.get('trending'):
            trending = resultado['trending'][:6]
            logger.info(f"IA gerou {len(trending)} trending topics")
            return trending

    except Exception as e:
        logger.error(f"Erro IA trending topics: {e}")

    return ['#BrasilAgora', '#Brasília', '#Brasil', '#Economia', '#Política', '#Tecnologia']
