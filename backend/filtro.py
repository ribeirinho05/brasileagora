"""
BRASIL AGORA — Filtro e Classificação de Notícias
...
logger = logging.getLogger('brasileagora.filtro')

FONTE_CATEGORIA_FORCADA = {
    'G1 Política': 'politica',
...
def _remover_duplicatas(noticias):
    ...
    for n in noticias:
        ...
        palavras = set(re.findall(r'\b[a-záàâãéèêíïóôõúüç]{4,}\b', titulo_limpo))

        duplicado = False
        for visto_titulo, visto_palavras in vistos:
            if titulo_limpo == visto_titulo:
                duplicado = True
                break
            if len(titulo_limpo) > 20 and len(visto_titulo) > 20:
                ratio = SequenceMatcher(None, titulo_limpo, visto_titulo).ratio()
                if ratio > 0.65:
                    duplicado = True
                    break
            if len(palavras) >= 4 and len(visto_palavras) >= 4:
                comuns = palavras & visto_palavras
                menor = min(len(palavras), len(visto_palavras))
                if menor > 0 and len(comuns) / menor > 0.6:
                    duplicado = True
                    break

        if not duplicado:
            vistos.append((titulo_limpo, palavras))
            resultado.append(n)
...
def _calcular_score(noticia):
    ...
    palavras_impacto = ['urgente', 'exclusivo', 'última hora', 'agora', 'breaking',
                        ...
                        'aprovado', 'vetado', 'impeachment', 'stf decide']
