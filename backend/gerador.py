"""
BRASIL AGORA — Gerador do news-data.js
"""
...
logger = logging.getLogger('brasileagora.gerador')
...
def gerar_news_data(noticias, videos, cotacoes, breaking_news, clima=None, trending=None):
    ...
    js = []
    js.append('/* BRASIL AGORA — Base de Dados v2.0')
    ...
    if clima:
        ...
        js.append(f"    cidade: '{_esc(clima.get('cidade','Brasília'))}', estado: '{_esc(clima.get('estado','DF'))}',")
    ...
    if trending and isinstance(trending, list):
        ...
        js.append("const trending = ['#BrasilAgora', '#Brasília', '#Brasil', '#Economia'];")
    js.append('')
