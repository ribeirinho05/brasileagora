/* BRASIL AGORA — Main JS (design nacional premium) */
(function() {
    'use strict';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        initClock(); initTheme(); initHero(); initTicker(); initQuotes();
        initTrending(); initCards(); initCatBlocks(); initVideos();
        initWeather(); initTopList(); initSideNews(); initSearch();
        initNav(); initShare(); initLoadMore(); initTabs();
    ...
    function initTheme() {
        ...
        var saved = localStorage.getItem('brasileagora-theme');
        if (saved) document.documentElement.setAttribute('data-theme', saved);
        if (btn) btn.addEventListener('click', function() {
            var t = document.documentElement.getAttribute('data-theme') === 'dark' ? 'light' : 'dark';
            document.documentElement.setAttribute('data-theme', t);
            localStorage.setItem('brasileagora-theme', t);
        });
    }
    ...
    function initHero() {
        ...
        repSlide.innerHTML = '<div class="hero-rep-split">' +
            ...
            '<span class="hero-slide-badge" style="background:#1B4332;color:#F4C430">Reportagem Especial</span>' +
            ...
            '<span class="hero-slide-meta">Redação Brasil Agora &bull; Reportagem</span>' +
            '</div></div>';
    ...
    function renderCards() {
        ...
        var repCard = curCat === 'todas' ? '<div class="card-h card-h-rep" onclick="window.location.href=\'reportagem-quantagropharma.html\'">' +
            ...
            '<span class="card-h-badge" style="background:#1B4332;color:#F4C430">Reportagem Especial</span></div>' +
            ...
            '<div class="card-h-excerpt">Rafael Erasmo de Oliveira Assis, 29 anos, criou método proprietário de simulação molecular quântica com aplicações no agronegócio e na indústria farmacêutica.</div>' +
            '<div class="card-h-meta"><span class="author">Redação Brasil Agora</span><span>Reportagem</span></div>' +
            '</div></div>' : '';
        el.innerHTML = repCard + show.map(function(n) {
            var ci = cats[n.categoria] || {nome: n.categoria};
            return '<div class="card-h" data-id="' + n.id + '">' +
                '<div class="card-h-img"><img src="' + n.imagem + '" alt="" loading="lazy">' +
                '<span class="card-h-badge">' + esc(ci.nome||n.categoria) + '</span>' +
                '<button class="card-h-share" data-title="' + esc(n.titulo) + '"><span class="material-icons">share</span></button></div>' +
                '<div class="card-h-body">' +
                '<div class="card-h-title">' + esc(n.titulo) + '</div>' +
                '<div class="card-h-excerpt">' + esc(n.resumo||'') + '</div>' +
                '<div class="card-h-meta"><span class="author">' + esc(n.autor) + '</span><span>' + n.tempo + '</span></div>' +
                '</div></div>';
        }).join('');
        var lb = document.getElementById('load-more');
        if (lb) lb.style.display = show.length < filtered.length ? 'block' : 'none';
        el.querySelectorAll('.card-h').forEach(function(c) {
            if (c.classList.contains('card-h-rep')) return;
            c.addEventListener('click', function(e) {
                if (e.target.closest('.card-h-share')) return;
                window.location.href = 'noticia.html?id=' + this.getAttribute('data-id');
            });
        });
        el.querySelectorAll('.card-h-share').forEach(function(b) {
            b.addEventListener('click', function(e) { e.stopPropagation(); openShare(this.getAttribute('data-title')); });
        });
    }

    function initCatBlocks() {
        var categories = ['politica','economia','brasilia','tecnologia','esportes','brasil','internacional'];
        var cats = typeof CATEGORIAS !== 'undefined' ? CATEGORIAS : {};
        var idsUsados = {};
    ...
    function initWeather() {
        var el = document.getElementById('weather-box');
        if (!el) return;
        var c = typeof clima !== 'undefined' ? clima : null;
        if (!c) { el.innerHTML = '<h3 class="widget-title">Clima</h3><p style="opacity:0.5">Indisponível</p>'; return; }
        el.innerHTML = '<h3 class="widget-title">Clima</h3>' +
            '<div class="w-main"><div class="w-temp">' + c.temperatura + '°</div><div class="w-icon"><span class="material-icons">' + (c.icone||'wb_sunny') + '</span></div></div>' +
            '<div class="w-desc">' + esc(c.condicao||'') + ' em ' + esc(c.cidade||'Brasília') + '</div>' +
            '<div class="w-details">' +
            '<span><span class="material-icons">thermostat</span> ' + (c.temp_min||'--') + '° / ' + (c.temp_max||'--') + '°</span>' +
            '<span><span class="material-icons">water_drop</span> ' + (c.umidade||'--') + '%</span>' +
            '<span><span class="material-icons">air</span> ' + (c.vento||'--') + ' km/h</span></div>';
    }

    function initTopList() {
        var el = document.getElementById('top-list-items');
        if (!el || !news.length) return;
        el.innerHTML = news.slice(0,5).map(function(n,i) {
            return '<div class="tl-item" data-id="' + n.id + '"><span class="tl-rank">' + (i+1) + '</span><div><div class="tl-title">' + esc(n.titulo) + '</div><div class="tl-time">' + n.tempo + '</div></div></div>';
        }).join('');
    ...
    function initSideNews() {
        ...
        if (!el || !news.length) return;
        var cats = typeof CATEGORIAS !== 'undefined' ? CATEGORIAS : {};
        var sideItems = news.slice(perPage, perPage + 15);
        el.innerHTML = sideItems.map(function(n) {
            ...
            return '<div class="side-news-item" data-id="' + n.id + '">' +
                ...
                '<div class="side-news-cat">' + esc(ci.nome || n.categoria) + '</div>' +
                '<div class="side-news-title">' + esc(n.titulo) + '</div>' +
                '<div class="side-news-meta">' + esc(n.autor) + ' &bull; ' + n.tempo + '</div>' +
                '</div></div>';
        }).join('');
    ...
    function initShare() {
        ...
        document.querySelectorAll('.share-item').forEach(function(b) {
            b.addEventListener('click', function() {
                var p = this.getAttribute('data-p'), t = ov.getAttribute('data-title')||'BRASIL AGORA', u = location.href;
